...
class WorkbookReader extends EventEmitter {
    ...
    for await (const entry of iterateStream(zip)) {
      ...
      switch (entry.path) {
        ...
        case 'xl/styles.xml':
          ...
          break;
        default:
          if (entry.path.match(/xl\/worksheets\/sheet\d+[.]xml/i)) {
          ...
          } else if (entry.path.match(/xl\/worksheets\/_rels\/sheet\d+[.]xml.rels/i)) {
            match = entry.path.match(/xl\/worksheets\/_rels\/sheet(\d+)[.]xml.rels/i);
            sheetNo = match[1];
  ...
  *_parseWorksheet(iterator, sheetNo) {
    ...
    const matchingRel = (this.workbookRels || []).find(
      rel => rel.Target.toLowerCase() === `worksheets/sheet${sheetNo}.xml`
    );
    const matchingSheet =
